...
#314159P = 3^15 5^2 P + 3^11 5^2 P + 3^8 5^1 P + 3^4 5^1 P - 3^0 5^0P
from callcryptolib import *
#from Join_double_base_number_system_base import *

def JointScalarMultiplication35(vect,coor,coor2,p):
	s1 = vect['s1']
	s2 = vect['s2']
	b = vect['b']
	t = vect['t']
	...
	for i  in range (0,m-1):
		...
		if i == 0 :
			if s1[i] == 1 and s2[i] == 1 :
				Z = quintuplingpow(v,T1,p)
			elif s1[i] == 1 and s2[i] == 0 :
				Z = quintuplingpow(v,coor,p)
			elif s1[i] == 0 and s2[i] == 1 :
				Z = quintuplingpow(v,coor2,p)
			elif s1[i] == 1 and s2[i] == -1 :
				Z = quintuplingpow(v,T2,p)
			elif s1[i] == -1 and s2[i] == 1 :
				Z = quintuplingpow(v,T3,p)
			elif s1[i] == -1 and s2[i] == -1 :
				Z = quintuplingpow(v,T4,p)
			elif s1[i] == -1 and s2[i] == 0 :
				Z = quintuplingpow(v,T5,p)
			elif s1[i] == 0 and s2[i] == -1 :
				Z = quintuplingpow(v,T6,p)
...
#	JDBC(3,5,a,b,True)
#	sb1, sb2, b2, c2 = JDBC(3,5,a,b)
#	vect = {'s1':sb1,'s2':sb2,'b':b2,'t':c2}
#	#print "punto P",coor,"\n"
#	t0=time.time()
#	Z=JointScalarMultiplication35(vect,coor,coor2,p)
#	t1=time.time()
...
#b :=  [2, 2, 2, 1, 0];
#t :=  [2, 1, 0, 0, 0];
#a :=  ( 1 ) * ( 3 ^ 2 * 5 ^ 2 )  +  (  0 ) * ( 3 ^ 2 * 5 ^ 1 )  +  ( 0  ) * ( 3 ^ 2 * 5 ^ 0 )  +  ( 0 ) * ( 3 ^ 1 * 5 ^ 0 )  +  ( 0 ) * ( 3 ^ 0 * 5 ^ 0 );
#k :=  ( 1 ) * ( 3 ^ 2 * 5 ^ 2 )  +  ( -1 ) * ( 3 ^ 2 * 5 ^ 1 )  +  ( -1 ) * ( 3 ^ 2 * 5 ^ 0 )  +  ( 1 ) * ( 3 ^ 1 * 5 ^ 0 )  +  (-1 ) * ( 3 ^ 0 * 5 ^ 0 );
#print " ";
#aux:=0;
#for x in s1 do 
#    aux:=aux + 1;
