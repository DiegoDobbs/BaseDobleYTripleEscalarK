...
def JointScalarMultiplication27(vect,coor,coor2,p):
	s1 = vect['s1']
	s2 = vect['s2']
	b = vect['b']
	t = vect['t']
...
#	sb1, sb2, b2, c2 = JDBC(2,7,a,b)
#	vect = {'s1':sb1,'s2':sb2,'b':b2,'t':c2}	
#	#print "punto P",coor,"\n"
#	t0=time.time()
#	Z=JointScalarMultiplication27(vect,coor,coor2,p)
#	t1=time.time()
...
#b :=  [4, 3, 2, 0];
#t :=  [1, 0, 0, 0];
#a :=  ( 1 ) * ( 2 ^ 4 * 7 ^ 1 )  +  ( 1 ) * ( 2 ^ 3 * 7 ^ 0 )  +  ( 0 ) * ( 2 ^ 2 * 7 ^ 0 )  +  ( 1 ) * ( 2 ^ 0 * 7 ^ 0 );
#k :=  ( 1 ) * ( 2 ^ 4 * 7 ^ 1 )  +  (-1 ) * ( 2 ^ 3 * 7 ^ 0 )  +  ( 1 ) * ( 2 ^ 2 * 7 ^ 0 )  +  ( 1 ) * ( 2 ^ 0 * 7 ^ 0 );
#print " ";
#aux:=0;
#for x in s1 do 
#    aux:=aux + 1;
